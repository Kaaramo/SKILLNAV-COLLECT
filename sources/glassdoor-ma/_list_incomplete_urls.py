"""Liste les URLs Glassdoor MA dont la description est < 200 chars."""
import json
import sys
...
POSTINGS = Path(r"F:\Web Mining Project\sources\collected\glassdoor-ma\postings")
OUTPUT = Path(r"F:\Web Mining Project\sources\collected\glassdoor-ma\raw\_incomplete_urls.json")
...
print(f"Total postings glassdoor-ma : {len(incomplete) + complete}")
...
n_with_url = sum(1 for r in incomplete if r['source_url'] and 'glassdoor' in (r['source_url'] or '').lower())
print(f"\nURLs Glassdoor valides : {n_with_url}/{len(incomplete)}")
